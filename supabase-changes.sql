...
-- =========================
-- CHAT: RESPOSTAS, REAÇÕES, FIXADOS E DENÚNCIAS
-- =========================
...
grant usage, select on sequence public.chat_message_reactions_id_seq to authenticated;
grant usage, select on sequence public.chat_message_reports_id_seq to authenticated;

drop policy if exists "chat reactions authenticated read" on public.chat_message_reactions;
create policy "chat reactions authenticated read"
...
drop policy if exists "chat pins authenticated read" on public.chat_message_pins;
create policy "chat pins authenticated read"
on public.chat_message_pins
for select
to authenticated
using (true);

drop policy if exists "chat pins own insert" on public.chat_message_pins;
create policy "chat pins own insert"
on public.chat_message_pins
for insert
to authenticated
with check (auth.uid() = pinned_by);

drop policy if exists "chat pins owner delete" on public.chat_message_pins;
create policy "chat pins owner delete"
on public.chat_message_pins
for delete
to authenticated
using (auth.uid() = pinned_by or public.is_admin(auth.uid()));

drop policy if exists "chat reports owner or admin read" on public.chat_message_reports;
create policy "chat reports owner or admin read"
...
-- =========================
-- PERFIL COM UPLOAD E MODERACAO DE VOZ
-- =========================

insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values ('profile-media', 'profile-media', true, 10485760, array['image/*'])
on conflict (id) do update
set
  public = excluded.public,
  file_size_limit = excluded.file_size_limit,
...
drop policy if exists "friendships select accepted public" on public.friendships;
create policy "friendships select accepted public"
on public.friendships
for select
to authenticated
...
add column if not exists can_moderate_voice boolean not null default false;

update public.community_roles
set can_moderate_voice = true
where can_manage_server = true
   or can_manage_roles = true;

create or replace function public.can_moderate_community_voice(
  server_id_input bigint,
  uid uuid
)
returns boolean
language sql
...
set search_path = public
as $$
  select coalesce(public.is_admin(uid), false)
    or exists (
      select 1
      from public.community_servers s
      where s.id = server_id_input
        and s.owner_id = uid
    )
    or exists (
      select 1
      from public.community_member_roles cmr
      join public.community_roles cr
        on cr.id = cmr.role_id
       and cr.server_id = cmr.server_id
      where cmr.server_id = server_id_input
        and cmr.user_id = uid
        and cr.can_moderate_voice = true
    );
$$;

create or replace function public.kick_voice_member(
  channel_id_input bigint,
...
begin
  ...
  select c.server_id
  into server_id_value
  from public.community_channels c
  where c.id = channel_id_input
    and c.channel_type = 'voice';

  if server_id_value is null then
    raise exception 'Canal de voz nao encontrado.';
  end if;

  if not public.can_moderate_community_voice(server_id_value, auth.uid()) then
    raise exception 'Sem permissao para moderar esta call.';
  end if;
...
grant execute on function public.can_moderate_community_voice(bigint, uuid) to authenticated;
grant execute on function public.kick_voice_member(bigint, uuid) to authenticated;

-- =========================
-- PERFIS PUBLICOS: COMENTARIOS E COLECOES
-- =========================
...
drop policy if exists "profile comments authenticated read" on public.profile_comments;
create policy "profile comments authenticated read"
on public.profile_comments
for select
to authenticated
using (true);

drop policy if exists "profile comments author insert" on public.profile_comments;
create policy "profile comments author insert"
on public.profile_comments
for insert
to authenticated
with check (author_id = auth.uid());

drop policy if exists "profile comments author or admin delete" on public.profile_comments;
create policy "profile comments author or admin delete"
on public.profile_comments
for delete
to authenticated
...
drop policy if exists "library public profile read" on public.library;
create policy "library public profile read"
on public.library
for select
to authenticated
using (true);

drop policy if exists "favorites public profile read" on public.favorites;
create policy "favorites public profile read"
on public.favorites
for select
to authenticated
using (true);

drop policy if exists "game play stats public profile read" on public.game_play_stats;
create policy "game play stats public profile read"
on public.game_play_stats
for select
to authenticated
using (true);
